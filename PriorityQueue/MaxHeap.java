...
public class MaxHeap implements MaxPriorityQueue {
	...
	/**
	 * put Max theElement into the heap
	 */
	public void putMax(Comparable theElement) {
	...
	public void putMin(Comparable theObject) {
		// increase array size if necessary
				if (size == heap.length - 1) {
					heap = (Comparable[]) ChangeArrayLength.changeLength1D(heap,2 * heap.length);
				}
				// find place for theElement
				// currentNode starts at new leaf and moves up tree
				int currentNode = ++size;
	...
	public Comparable removeMin() {
		...
		Comparable minElement = heap[1];
		Comparable firstElement = heap[size--];
		// find place for lastElement starting at root
		int currentNode = 1,
		// child of currentNode
		child = 2;
		while (child <= size) {
			...
			// can we put lastElement in heap[currentNode]?
			if (firstElement.compareTo(heap[child]) <= 0) {
				break; // yes
			}// no
			heap[currentNode] = heap[child]; // move child up
			currentNode = child;
